...
function Invoke-StrictNoRedirectProbe([string]$BaseUrl, [string]$CAPath, [string]$Path) {
    $trustedCA = [System.Security.Cryptography.X509Certificates.X509Certificate2]::new($CAPath)
    $previousValidator = [System.Net.ServicePointManager]::ServerCertificateValidationCallback
    [System.Net.ServicePointManager]::ServerCertificateValidationCallback = {
        ...
        $leaf = [System.Security.Cryptography.X509Certificates.X509Certificate2]::new($certificate)
        try {
    ...
    try {
        ...
        [System.Net.ServicePointManager]::ServerCertificateValidationCallback = $previousValidator
        $trustedCA.Dispose()
    }
}
...
function Assert-LegacyPathRejected([string]$BaseUrl, [string]$CAPath, [string]$Path) {
    $response = Invoke-StrictNoRedirectProbe $BaseUrl $CAPath $Path
    Assert-Status $response 404 "Legacy endpoint $Path"
    if (-not [string]::IsNullOrEmpty($response.Location)) {
        throw "Legacy endpoint $Path unexpectedly returned Location $($response.Location)."
    }
}
...
try {
    ...
    Assert-LegacyPathRejected $baseUrl $caPath "/api/v1/auth/initial-admin"
    Assert-LegacyPathRejected $baseUrl $caPath "/api/v2/auth/initial-admin"
    ...
    $response = Invoke-AgentJSON $baseUrl $caPath "POST" "/api/auth/initial-admin" @{
    ...
    $response = Invoke-AgentJSON $baseUrl $caPath "POST" "/api/auth/login" @{
    ...
    $response = Invoke-AgentJSON $baseUrl $caPath "PUT" "/api/config/session" @{ idleTimeoutSeconds = 180 }
    ...
    $response = Invoke-AgentJSON $baseUrl $caPath "GET" "/api/config/session" $null
    Assert-Status $response 200 "Idle timeout read"
    if ($null -eq $response.Json -or $response.Json.idleTimeoutSeconds -ne 180) {
        throw "Idle timeout read did not return the persisted 180 seconds."
    }

    foreach ($retiredPath in @("/api/auth/activity", "/api/auth/logout")) {
