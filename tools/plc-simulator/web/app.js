...
const emptyPoints = document.querySelector("#empty-points");
const message = document.querySelector("#message");
const editor = document.querySelector("#editor");
const form = document.querySelector("#point-form");
const editorTitle = document.querySelector("#editor-title");
const nameInput = document.querySelector("#point-name");
const typeInput = document.querySelector("#point-type");
const addressInput = document.querySelector("#point-address");
const descriptionInput = document.querySelector("#point-description");
const writableInput = document.querySelector("#point-writable");
let points = [];
let editingID = null;
...
function sourceText(source) {
    return { modbus: "机器 Modbus", ui: "页面写入", definition: "点位定义", initial: "初始" }[source] ?? source;
}
...
async function loadPoints() {
    points = await request("/api/points");
    renderPoints();
}
async function loadStatus() {
    const status = await request("/api/status");
    document.querySelector("#modbus-address").textContent = `${status.modbusAddress}（Unit ${status.unitId}）`;
    document.querySelector("#active-connections").textContent = String(status.activeConnections);
}
...
async function writeValue(point, value) {
    try {
        await request(`/api/points/${encodeURIComponent(point.id)}/value`, "PUT", { value });
...
async function deletePoint(point) {
    ...
    try {
        await request(`/api/points/${encodeURIComponent(point.id)}`, "DELETE");
...
form.addEventListener("submit", async (event) => {
    ...
    try {
        if (editingID === null) {
            await request("/api/points", "POST", input);
            setMessage("点位已添加", true);
        }
        else {
            await request(`/api/points/${encodeURIComponent(editingID)}`, "PUT", input);
...
document.querySelector("#add-point").addEventListener("click", () => showEditor());
document.querySelector("#cancel-edit").addEventListener("click", hideEditor);
